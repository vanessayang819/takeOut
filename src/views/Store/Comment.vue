    ...
    <div ref="comment" class="scroll-container">
      <article>
        <div class="comment-score-container">
          <div>
            <span class="comment-score">{{poiInfo.wm_poi_score}}</span>
            <h3>商家评分</h3>
          </div>
          <div>
            <div>
              <span>口味</span>
              <span class="food-score">{{poiInfo.food_score}}</span>
            </div>
            <div>
              <span>包装</span>
              <span class="pack-score">{{poiInfo.pack_score}}</span>
            </div>
          </div>
          <div>
            <span class="delivery-score">{{poiInfo.delivery_score}}</span>
            <h3>配送评分</h3>
          </div>
        </div>
        <!--评分类型部分-->
        ...
        <article class="comments-container">
          <section v-for="item in commentData" :key="item.id">
            ...
            <div class="comment-main-part">
              ...
              <div class="order-comment-score"><span>评分 </span>
              </div>
  ...
  export default {
    ...
    mounted() {
      ...
      getRestaurant({restaurant_id: _this.restaurant_id}).then((response) => {
        _this.poiInfo = response.data.data;
      })
    },
    methods: {
